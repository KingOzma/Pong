...
-- Initialzies sphere's velocity based on the person who scored last.
        sphere.dy = math.random(-25, 25)
        if servingPlayer == 1 then
            sphere.dx = math.random(140, 200)
        else
            sphere.dx = -math.random(140, 200)
        end
            sphere_detected_positions = {}

    elseif gameState == 'play' then

-- Number of cases to take before making the position prediction
            cases = 10
-- Get the sphere's current position
            current_sphere_position = {x = sphere.x, y = sphere.y}
        
        if table.getn(sphere_detected_positions) < cases then
            table.insert(sphere_detected_positions, current_sphere_position)
        else -- estimate the angle
            p1 = sphere_detected_positions[cases-1] -- second last position
            p2 = sphere_detected_positions[cases] -- last position
-- Calculate the sphere velocity
            velocity = {
                x = p2.x - p1.x, 
                y = p2.y - p1.y 
            }
        ...
        if  sphere.dx < 0 then
-- Update the position according to the sphere velocity
            if player1.y > sphere.y then
                player1.dy = -Racquet_Speed
            elseif player1.y < sphere.y then
                player1.dy = Racquet_Speed
            end        
        else
-- Move the racquet in the middle of the y axis
            if player1.y + player1.height/2 < Super_Height/2 then
                player1.dy = Racquet_Speed
            elseif player1.y + player1.height/2 > Super_Height/2 then
                player1.dy = -Racquet_Speed
            else
                player1.dy = 0
            end
...
-- AI for player 2 
        if player2.y > sphere.y then
            player2.dy = -Racquet_Speed
        elseif player2.y < sphere.y then
            player2.dy = Racquet_Speed
        end
...
-- Predict final position
            angle = math.atan2(sphere.dy, sphere.dx) -- negative if sphere is going up, positive if sphere is going down
            
            final_y = sphere.y + (Super_Width - player1.width*2 - 20)*math.tan(angle)
            final_x = Super_Width - 10

            if angle < 0 then
                if final_y < sphere.y and final_y > 0 then
                    print("UP")
                else 
                    print("Going to hit the wall")
                end
            else
                if final_y > sphere.y and final_y < Super_Height then
                    print("DOWN")
                else
                    print("Going to hit the wall")
                end 
...
--If the score of 5 is reach the game is over and victory message is shown.
    ...
    --Player 1 Movenment
    --if Ai_mode == false then (Uncomment for AI control add another end below line 258)
    ...
    --Player 2 Movenment
    if Ai_mode == false then --Comment out for player control
    ...
    end -- Comment out for player control
